...
pub enum McrCpCmdSet {
    Generic = 0,
    /// Large-Data-Transfer command set (Martichoras PR #16771989).
    /// When set, driver + firmware interpret the SQE's `src_data`
    /// union as `metadata_with_session` and DMA-read the
    /// Metadata Page. Matches `CP_CMD_SET_DATA_XFER` in the driver
    /// UAPI (`azihsm_hsm_dev_ioctl.h`).
    DataXfer = 1,
}
...
// The driver takes a list of {user_vaddr, xfer_length} pairs, pins the
// user pages, builds one NVMe SGL segment per buffer, then builds a
// 4 KB Metadata Page whose {u32 xfer_length, u32 rsvd, u64 sgl_paddr}
// entries point at those segments. The device DMA-reads the Metadata
// Page from `metadata_page_addr` in the SQE's `metadata_with_session`
// src_data variant. All of that is kernel-side — userland just
// hands over the buffer list.
// ─────────────────────────────────────────────────────────────────────

/// Maximum number of buffers per data-transfer ioctl (matches
/// `AZIHSM_MAX_DATA_XFER_BUFFERS` in `azihsm_hsm_dev_ioctl.h`).
pub const AZIHSM_MAX_DATA_XFER_BUFFERS: usize = 16;

/// Maximum per-buffer transfer length (matches
/// `AZIHSM_MAX_DATA_XFER_PER_BUFFER` in `azihsm_hsm_dev_ioctl.h`).
pub const AZIHSM_MAX_DATA_XFER_PER_BUFFER: u32 = 64 * 1024;

/// One `{xfer_length, buf_addr}` slot passed to the driver — kernel
/// walks the buffer, pins pages, builds an SGL segment for it.
#[repr(C)]
#[derive(Copy, Clone)]
pub struct AziHsmDataxferBufferSlot {
    xfer_length: u32,
    buf_addr: *const u8,
}

impl Default for AziHsmDataxferBufferSlot {
    fn default() -> Self {
        Self {
            xfer_length: 0,
...
/// Full `azi_hsm_dataxfer_buffers` structure passed to the driver.
///
/// Layout (must be byte-identical to the C UAPI):
/// ```text
///   u32                          buffer_cnt
///   [{u32, *u8}; 16]             buffers
///   [u32; 128]                   rsvd (padding for forward compat)
/// ```
#[repr(C)]
pub struct AziHsmDataxferBuffers {
    buffer_cnt: u32,
    buffers: [AziHsmDataxferBufferSlot; AZIHSM_MAX_DATA_XFER_BUFFERS],
    rsvd: [u32; 128],
}

impl Default for AziHsmDataxferBuffers {
    fn default() -> Self {
        Self {
            buffer_cnt: 0,
            buffers: [AziHsmDataxferBufferSlot::default(); AZIHSM_MAX_DATA_XFER_BUFFERS],
            rsvd: [0u32; 128],
        }
    }
}
...
#[repr(C)]
#[derive(Default)]
pub struct McrCpDataXferCmd {
    generic_cmd: McrCpGenericCmd,
    dataxfer_buffers: AziHsmDataxferBuffers,
}

/// `set` field value that signals the driver + firmware to treat the
/// SQE's `src_data` union as `metadata_with_session` and DMA-read the
/// Metadata Page. Matches `CP_CMD_SET_DATA_XFER` in
/// `azihsm_hsm_dev_ioctl.h`. Kept as a bare constant for anywhere
/// that touches the wire value directly; `McrCpCmdSet::DataXfer`
/// carries the same numeric encoding for typed code paths.
#[allow(dead_code)]
const CP_CMD_SET_DATA_XFER: u16 = 0x1;
...
impl DdiDev for DdiNixDev {
    ...
    fn exec_op_tbor<T: TborOpReq>(
        ...
        for item in oob_slice.iter() {
            if item.len() as u64 > AZIHSM_MAX_DATA_XFER_PER_BUFFER as u64 {
                return Err(DdiError::InvalidParameter);
            }
        }
        ...
        let (status, ioctl_status, byte_count) = if use_data_xfer {
            ...
            cmd.dataxfer_buffers.buffer_cnt = oob_slice.len() as u32;
            for (i, item) in oob_slice.iter().enumerate() {
                cmd.dataxfer_buffers.buffers[i] = AziHsmDataxferBufferSlot {
                    xfer_length: item.len() as u32,
                    buf_addr: item.as_ptr(),
                };
            }
